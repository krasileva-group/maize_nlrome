...
'%ni%' <- Negate('%in%')
setwd("~/Dropbox/MaizeNLRome/NLRome/")
...
x <- left_join(w,bs)
x
x %>% filter(label == "Int1300")
z %>% filter(label == "Int1300")
...
good_size_clades
good_size_clades %>% filter(bootstrap <70) %>% arrange(N_tips) %>% print(n=50) ## 12 poor clades
...
missing ## only sorghum sequences are missing from partition. These can be ignored.
